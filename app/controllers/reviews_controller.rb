    ...
    if @review.save
      if current_user.organization?
        ...
        if Review.where("organization_id = ? AND author_id != ?", @project.organization.id, @project.organization.id).count >= 3 && Review.where("organization_id = ? AND author_id != ?", @project.organization.id, @project.organization.id).average(:rating) > 4.5
          MyBadge.create(user_id: @project.organization.user.id, badge_id: Badge.find_by(name: "Top Rated").id).save! if  MyBadge.where("user_id = ? AND badge_id = ?", @project.organization.user.id, Badge.find_by(name: "Top Rated").id).length == 0 # This line is for Top Host badge (see seed file)
        else
